...
//        execute(conn, String.format(template, tableName, tableColumn.toAdd()));
    ...
    public static void renameColumn(
        ...
        execute(
                ...
                String.format(template,
                              tableName,
                              oldTableColumn.getName().trim(),
                              "'" + newTableColumn.getName().trim()
                             ) + "'"
               );
    }
    ...
    private static ResultSet selectWithoutSubrequest(
            Table table,
            List<TableColumn> columns,
            WhereValues where,
            Connection conn
    ...
    private static ResultSet selectWithSubrequest(
            Table table,
            List<TableColumn> columns,
            WhereValues where,
            Connection conn
                ...
                if (foreignKeyColumn instanceof AutoincrementColumn) {
                    ...
                    for (TableColumn columnForeignKeyTable : collect) {
                        ...
                        subRequest.append(
                                String.format(
                                        selectTemplate,
                                        String.format(templateSqlName),
                                        columnForeignKeyTable.getName(),
                                        //Наименование колонки внешней таблицы
                                        (!foreignKeySelf)
                                        ? (String.format(templateSqlName, pseudoName))
                                        : (String.format(templateAs, foreignKeyTable.getName(), pseudoName)),
                                        //Наименование внешней таблицы
                                        String.format(templateFullName, pseudoName, foreignKeyColumn.getName()) +
                                        " = " + String.format(templateFullName, col.getTable().getName(), col.getName())
                                        //Блок WHERE
                                             )
                                         );
                    ...
                    subRequest.append(String.format(
                            ...
                            ? (String.format(templateSqlName, pseudoName))
                            : (String.format(templateAs, foreignKeyTable.getName(), pseudoName)),
                            String.format(templateFullName, pseudoName, fkTableColId.getName()) + " = " +
                            String.format(templateFullName, col.getTable().getName(), col.getName())
                                                   ));
